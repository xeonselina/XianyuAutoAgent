...
**数据库配置**:
```env
DATABASE_URL=<set-via-secret-store>
```
...
# 顺丰快递
SF_PARTNER_ID=<set-via-secret-store>
SF_CHECKWORD=<set-via-secret-store>
SF_SENDER_NAME=张女士
...
# 快麦打印
KUAIMAI_APP_ID=<set-via-secret-store>
KUAIMAI_APP_SECRET=<set-via-secret-store>
KUAIMAI_PRINTER_SN=<set-via-secret-store>
```
