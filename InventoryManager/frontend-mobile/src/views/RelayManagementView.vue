...
async function refreshAll() {
  ...
  try {
    let totalCount = 0
    let successCount = 0
    for (let offset = 0; offset < refreshableIds.value.length; offset += 100) {
      const result = await refreshRelayTrackingBatch(
        refreshableIds.value.slice(offset, offset + 100),
      )
      totalCount += result.total
      successCount += result.success_count
    }
    showSuccessToast(`已刷新 ${successCount}/${totalCount} 条`)
    await loadCases()
  } catch (error) {
