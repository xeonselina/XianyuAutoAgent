...
自动化测试 SHALL（必须）不加载生产 `.env`、不连接生产业务库，也不使用名称中不含 `test` 的数据库。MySQL 事务测试优先使用隔离容器和合成数据，也可以连接 `192.*` MySQL 实例中名称包含 `test` 的独立测试库，但测试账号必须只能访问该测试库。

#### Scenario: 不安全的测试数据库地址

- **WHEN** 测试配置指向生产业务库、非测试数据库名，或者没有设置 `TESTING=true`
- **THEN** 测试启动程序必须在打开数据库连接前终止

#### Scenario: 使用生产实例中的独立测试库

- **WHEN** `TEST_DATABASE_URL` 指向 `192.*` MySQL 实例中名称包含 `test` 的独立数据库，并使用只能访问该库的专用账号
- **THEN** 自动化测试可以连接并使用该测试库
- **AND** 不得查询或修改同一实例中的生产业务库

#### Scenario: 确实需要接近生产的数据

- **WHEN** 合成 fixture 无法复现问题，且获得单独授权
- **THEN** 必须以只读单事务方式 dump 数据并导入独立测试库后才能运行测试
- **AND** dump 不强制脱敏，但不得提交 Git 或对外暴露
