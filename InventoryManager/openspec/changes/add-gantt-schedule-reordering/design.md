...
测试必须显式设置 `TEST_DATABASE_URL` 和 `TESTING=true`，数据库名必须包含 `test`。MySQL 专属测试优先使用隔离容器，也可以使用 `192.*` MySQL 实例中的独立测试库，但测试账号只能访问该测试库。生产 dump 只能在单独授权后以只读方式获取，导入测试库后不强制脱敏，但不得进入 Git 或对外暴露。
