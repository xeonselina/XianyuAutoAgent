...
## Status
Implemented; ready for archival after strict validation
...
- 半日达(expressTypeId=263):当日/次日达,适合本地配送
...
- 类型: `INTEGER`
- 应用默认值: `2` (标快); 初始迁移将历史 `NULL` 回填为 `2`
- 可选值: `1`(特快), `2`(标快), `263`(半日达)
- 用途: 存储用户为该订单选择的快递类型
...
- 运单号已存在或订单已预约/发货后拒绝修改，避免本地选择与承运商订单不一致
...
- 半日达同时传递 `specialDeliveryTypeCode=263`; 非法存量值不得调用 provider
...
1. **数据迁移风险**: 现有租赁记录没有 `express_type_id` 字段
   - 缓解: 初始迁移将历史空值回填为2,应用对缺失值使用2；其他非法值 fail closed

2. **用户误操作风险**: 用户可能不小心选错快递类型
   - 缓解: 设置合理默认值(标快),在UI上突出显示当前选择；运单创建后锁定
...
3. ✅ **历史数据处理**: 已发货的订单是否需要回填 `express_type_id` 字段?
   - 回答: 初始迁移将空值回填为2，但不声称该值还原了历史真实产品类型
...
- ✅ 已存在的租赁记录使用默认值(标快)
- ✅ 运单创建后服务端拒绝修改快递类型，前端同步禁用选择器
- ✅ 半日达代码在提案、规格、任务、UI、模型和顺丰报文中统一为263
