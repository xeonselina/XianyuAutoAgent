...
- `添加设备`, `待归还`, and `客户历史` remain direct secondary actions; the return count stays visible.
- All less frequent actions use one `更多操作` menu in this order: operational work, schedule tools, then secondary business pages.
- The fixed row/column toolbar is replaced by a responsive CSS grid that moves the action group to its own row before individual buttons wrap.
