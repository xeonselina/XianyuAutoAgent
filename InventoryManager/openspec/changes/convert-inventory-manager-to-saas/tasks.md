...
- D01–D73 以当前决策表和 superseding 记录为准；D63 取消迁移冻结，D64 将项目流程简化为实现、验证、记时、演练、切换和观察，D65–D67 将 SQL-backed 测试简化为串行复用现有 `inventory_management_test`；D68 将默认租户旧运单/打印事实迁为独立 `legacy_unattributed` 只读历史；D69 用精确 manifest、主订单权威和 fail-closed unavailable 规则关闭剩余代表性 backfill 输入边界；D70 要求保留领域复杂度但统一工程骨架；D71 明确 SQL-backed 仅支持 MySQL 8 与 MariaDB、完全不支持 SQLite；D72 选择不改产品行为与 schema 语义的深度收缩方案 B，并禁止用复杂通用框架换取表面减量；D73 允许一个租户关联多个闲管家账号/闲鱼店铺，并要求告警、rental 和 provider job 始终保留 connection 归属。历史文字与当前规则冲突时不得恢复已被取代的流程。
- 实施期间保留用户已有改动，不处理、覆盖或提交与本 change 无关的文件。
- 2026-08-24 已完成 D73 文档同步但尚未开始实现；按项目负责人要求，当前全部实现工作暂停，等待明确恢复指令。
...
- [ ] 9.10 实现闲鱼 credential/cursor/sync state tenant-bound、每租户每 3 分钟同步、显式即时高优先级同步和 180 秒幂等合并。
  - [x] D73 已确认并写入决策、设计和 delta specs：一个租户可有多个闲管家账号/闲鱼店铺，一个 Xianyu integration 对应一个账号/店铺；租户级 scheduled/manual refresh 覆盖全部 active connection，订单与告警按 connection + order number 归属，不新增平行账号 aggregate。
  ...
  - [ ] 按 D73 把 alert 唯一/忽略/替换从裸 `order_no` 收敛为 connection-scoped identity，返回 tenant-local alert ID 与店铺标签，聚合 UI 显示/筛选店铺；保留一个租户级全 active 店铺刷新，不增加逐店铺 provider job。
- [ ] 9.11 将 add-scheduled-shipping-status、add-single-rental-ship-button、add-xianyu-missing-order-alerts、enable-selective-batch-shipping、integrate-xianyu-order-api、interleave-shipping-slips-with-waybills、simplify-batch-shipping-flow、view-sf-shipment-tracking 重基到 warehouse context、持久 job、执行快照和统一幂等状态机。
  - [ ] 按 D73 给 rental/创建编辑 DTO 增加可空 Xianyu connection 归属：告警建单自动继承、单 active 店铺可自动选、多店铺手工录单必须选；订单详情查询和闲鱼发货必须从所选 connection 解析 current verified revision，并在 provider 前冻结 exact revision，禁止 direct HTTP、跨店铺 fallback 和 intent 后改写归属。
...
- [ ] 12.2 实现版本锁定、可 dry-run、可中断续跑且具有稳定 migration idempotency key 的默认租户迁移命令与 manifest；命令明确区分 expand、backfill/verify、application enforce、database/jobs enforce、rollback-before-authoritative-write 和 contract，各阶段输出前置、完成证据、停止条件和不可逆边界，非生产验证不得连接生产写身份或调用真实 provider/打印机。
  ...
  - [x] 已把附件 `backup_20260824_080001.sql` 中唯一 `inventory_management` 段恢复到 Docker MySQL 8 的精确 `inventory_management_test`，以 schema-only `SELECT/SHOW VIEW` 身份连续两次得到稳定的新 source identity：12 表、8,536 行、source snapshot digest `2c03aaad9323dfe788113ce2cbd19ab302ecf21f2451dc76c7291fe5ea592313`、D68 boundary digest `b7fd5bf39ef7fc2063051c54edf523555c51236a39977e291b7116f818517c27`。当前 205 文件 bundle 和受控演练输入生成代表性 manifest 后，forward expand、原地登记、默认仓、三类 metadata-only integration、D68、12 类 reconciliation 与精确重放聚焦运行 `1 passed in 11.91s`；没有创建 credential revision/current pointer、Core shipment/attempt/print ledger 或 provider/打印副作用。
  - [x] D69 已关闭代表性快照的剩余输入边界：manifest 只批准 Rental 778 的 `6 → 2（标快）`，主订单权威生成物流/附件新字段，结构化地址与逻辑附件按确定性规则自动 resolved 或显式 unavailable；完整 resolved backfill、D68、12 类 reconciliation、精确重放和负向约束已在同一 MySQL 8 快照通过。此前 9 表/6,264 行 baseline 只保留为历史证据，不得复用旧 digest。
  - [ ] 仍需把该代表性 resolved backfill 纵向切片接入 `run-to-authoritative-boundary` 的完整命令并固定生产受控 identity/image 输入；这不再等待人工逐行计划或 express 处置。
- [ ] 12.3 实现原地登记工具：新建或迁移 `inventory_control`，为现有公司生成不可变 tenant/database UUID，在原业务 schema 写入唯一 `database_identity` 并登记可信 route；不得复制整库、重编号主键、重命名 schema 或给每张业务表添加 `tenant_id`。默认租户显示名称和首位 Admin 手机号必须由受控输入显式提供，拒绝空值、占位值、非法/歧义手机号和同 idempotency key 下不一致的重跑输入，且日志不得回显敏感值。
  ...
  - [ ] 代表性演练已用固定显示名称/首位 Admin/tenant/database UUID 与当前 bundle 生成并重放 manifest，但这些只是非生产 harness 输入；仍需固定生产受控输入和 implementation/image identity，通过正式 CLI 生成实际 manifest 并运行完整命令。由 current head downgrade 得到的结构 fixture 仍不能被误称为完整历史空库重建，生产业务库继续只允许只读观察。
- [ ] 12.4 实现按默认仓/设备位置、结构化地址、逻辑附件 unit/request/link/event、计划物流字段、integration/credential revision、shipment/print snapshot 的幂等 backfill 和反向核对；canonical express-type 的历史 NULL 回填为 `2`，并只允许 D69 精确 manifest 绑定的 Rental 778 把 legacy `6` 转为标快 `2`；其他 `6`/非法值生成逐状态报告并保持 provider fail-closed，绝不映射为 `263` 或提交顺丰。迁移 manifest 必须拒绝把 D61 暴露旧值仅加密成新 revision，只接受已轮换并重新验证的 provider revision 引用。
  - [ ] 按 D73 扩展默认租户 backfill/reconciliation：旧 rental 只有闲鱼订单号而无可信店铺来源时保留 `xianyu_integration_uuid=NULL`，不得用唯一 current connection、后来验证的 revision、显示名或相似告警自动归属；本地展示保留，首次 provider 动作前由 Admin/Operator 显式选择店铺。
  - [x] 已实现默认仓/设备位置、计划物流字段和 canonical express-type 幂等 tenant transaction backfill；计划物流使用完整 main/child source commitment 并共享 D33 计算，新字段只由主订单权威生成，旧 child 日期/状态不改写；express-type policy revision 2 只允许 `NULL → 2` 与 manifest 精确批准的 `6 → 2`，未获批 `6`/unsupported 仍阻止 provider-ready，服务均不调用 provider。
  - [x] 已实现显式 legacy-device→logical-unit 计划：只把手机支架/三脚架的唯一 legacy source 创建为隐藏 unit，所有未终结附件 child rental 完整 upsert request，可靠映射才创建带主订单 planned window 的 link；无可靠 link 的 shipped/returned request 仅在 source unit 保持 maintenance 时以 request-only fail-closed 形式保存，旧 child 事实不改写。重跑不重复 unit/request/link/event，手柄不进入逻辑库存。
  ...
  - [x] 已实现 D69 结构化收货地址计划：只在旧 `destination` 含可精确提取的省/市/区/详情连续序列时自动生成目标 commitment；空值或不可可靠解析的行生成显式 unavailable commitment、保留旧展示字段并保持 provider fail-closed。源值、父子关系、半回填或目标变化时整批回滚，精确重跑不改写。
  ...
  - [x] 代表性快照的 D68 非空 adapter 已在 MySQL 8 实际运行：2,104 个 rental 生成 2,104 条独立 shipment snapshot、0 条 print snapshot，第二次运行 result digest 完全一致且为 no-op；源漂移和临时 Core shipment ledger 均被拒绝并事务回滚，最终 source-rental 孤儿/重复、Core shipment/attempt/print、credential revision 和 integration current pointer 全为零。历史没有绑定后来凭证。
  - [x] D69 代表性非空 resolved backfill 已在 MySQL 8 完成：Rental 778 的 `6` 与 2 条 NULL 均转为标快 `2`；262 条活动主订单生成计划物流，178 条地址 resolved、84 条 unavailable；12 个逻辑附件 unit 中 8 个可靠、4 个 maintenance，24 个 request 中建立 8 个可靠 link。D68 同轮生成 2,104 条只读历史；精确重放、源漂移、孤儿、重复和 Core 执行账本非零负向验证通过，并已由完整命令消费。12.4 主项只因生产受控 identity/image 输入及 D61 后重新提交、验证的新 credential revisions 尚未固定而保持开放；不得给旧历史回填这些 revision。
...
- [x] 12.6 实现控制库、默认 tenant schema 和迁移报告的自动对账，覆盖逐表行数、金额、设备/租赁/附件关联、孤儿数据、历史运单、凭证 revision、默认仓、legacy 双重计数、schema generation/digest 和预先定义阈值；任一未知、漂移或非零异常没有显式 disposition 时，迁移命令返回失败且不得继续写入。
  ...
  - [x] 已在版本锁定的非空代表性 MySQL 8 快照上组合相同 policy、SQL registry、只读 schema observer 与 legacy-authority observer；当前已完成切片的 12 个 scope 全部 matched：139 个设备/仓库关联、2,104 个 rental/device 关联、2,079 条历史 waybill、金额 minor-unit 总和、0 credential revision、0 orphan、0 legacy double count、唯一默认仓以及 schema generation/digest 均通过。
  - [x] D69 代表性最终 backfill 状态已由完整 `DefaultMigrationCommand` 消费同一 12 类 reconciliation：逻辑附件 link 期望/实际均为 8，默认仓、设备/租赁关系、金额、历史 waybill、credential revision、孤儿、legacy double count、schema generation/digest 全部 matched；只有报告通过才写入 backfill phase completion 并继续 application/database-jobs enforce。随后源漂移和 Core ledger 注入都在事务内被拒绝并回滚。
- [ ] 12.7 在合成 fixture 和隔离的脱敏代表性快照上反复执行迁移、幂等重跑、每阶段崩溃续跑、预权威写回滚及 tenant-aware forward-fix 验证；针对 MariaDB 10.11/`utf8mb3` 源到 Core MySQL 8 目标验证 DDL、数据类型、SQL mode、timezone、collation/`utf8mb4` 转换、行锁/`SKIP LOCKED`、advisory lock、dump/restore 和 connector 行为，不把该验证称为首次生产规模演练。若使用本机可达的当前生产库，只允许经 grants 证明的目标 schema `SELECT/SHOW VIEW` 和单条非锁定只读观察；所有写入、迁移、fixture reset 与破坏性注入必须显式设置 `ALLOW_REAL_TEST_DATABASE=true`，且只允许名称精确为 `inventory_management_test` 的库，并先观察其可能漂移的实际 schema 后再按用例 fail closed、显式重建或迁移。
  ...
  - [x] 实际非空 D68 子矩阵已覆盖 dump 段白名单恢复、MySQL 8 `20260807_damage_notes → 20260824_legacy_history` forward expand、digest-pinned shared advisory lock、source identity/manifest 重放、D68 首次/精确幂等运行、12 类对账、源漂移拒绝和 Core ledger 非零拒绝；完整代表性用例 `1 passed in 11.91s`。所有写入仅发生在 Docker 的 `inventory_management_test`，未连接生产业务库或外部 provider/打印机。
  - [ ] 12.7 主项已经覆盖 D69 resolved plans 的最终完整 pre-authority 命令和同 journal 空操作重跑；仍需补齐各阶段独立崩溃续跑、预权威写旧应用回滚、tenant-aware forward-fix 及固定 identity 的完整可复现链。legacy express type 只允许 manifest 精确绑定的 Rental 778 执行 `6 → 2`，其他 `6` 继续 fail closed。
- [ ] 12.8 验证迁移/回滚 bundle、manifest、固定输入 schema、对账器和非生产测试均版本锁定且可从空环境复现；确认测试 harness 对生产 URL、非精确测试库名、越权 grants、锁定/文件 SQL 和 schema 漂移均 fail closed，没有生产数据库 mutation、真实短信/SF/快麦/闲鱼提交或物理打印，所有已知数据异常均已修正或有 fail-closed disposition。
  ...
  - [x] D69/D70 代表性完整命令使用当前 207 文件 bundle 在 Docker MySQL 8 的精确 `inventory_management_test` 通过：四个 pre-authority phases 依序完成并停在 contract，application enforce 写入唯一 D60 grant 并发布 active/ready，database/jobs evidence 保持零生产写身份和零 provider/打印副作用；相同 journal 重跑执行零 phase。最终运行 `1 passed in 11.94s`，之后从获准 dump 恢复 12 表/8,536 行 source baseline，三个 source digest、D68 boundary digest、旧 head 和空闲 advisory lock 全部复核一致。
...
## 14. D70/D72 统一架构与深度代码规模治理

**执行时点：** 立即执行。14.3 先关闭当前 D69 代表性迁移命令检查点；该检查点确认后，除修复回归和解除既有阻塞外，先完成 14.4–14.6，再继续增加新的产品能力。不得用本节改变 D01–D69 的产品行为或安全边界。

**完成条件：** control-plane/tenant-plane 与必要领域状态机保留，但全项目领域包、事务、错误、证据/摘要、幂等、HTTP、worker/job、repository/CAS 和测试 fixture 使用统一结构；重复实现被真实删除，行为、迁移安全约束和有效覆盖不下降。

**可验证结果：** 每个收敛切片记录 production/test LOC 前后值、被替代的重复点、唯一权威入口和聚焦回归；D72 以远端 `main...saas-main` 的 265,190 行新增、backend runtime 139,663 行和 tests 94,400 行为修正基线，目标 backend runtime 减少约 5–6.5 万行、tests 减少约 3–4 万行、总新增差异约 16–18 万行。迁移历史、有效覆盖、安全约束和数据库行为不得为达标而删除。

- [x] 14.1 固定 2026-08-24 只读结构审计基线：`main...saas-main` 为 891 个文件、266,306 行新增、5,032 行删除，其中测试新增约 94,675 行、`inventory_control` 约 86,508 行、`app` 约 53,335 行、前端约 11,040 行、control migrations 约 8,931 行、OpenSpec 约 6,478 行；`inventory_control` 与 `app` 生产 Python 当前约 141,966 行。记录 396 个 Error 类、534 个 dataclass、102 处 `SessionTransactionOrigin` 检查、30 个 `_canonical_digest`、12 个 `_run_in_transaction`、4,191 行 `__init__.py`、89 个不少于 500 行和 25 个不少于 1,000 行的生产文件，作为重复收敛线索而非机械删除指标。
- [x] 14.2 确认统一结构和代码预算：领域包只使用 `domain.py`、`service.py`、`repository.py`、可选 `http.py`/`worker.py`、`composition.py`；跨领域只保留一套 errors/transactions/evidence/state_machine/idempotency/jobs；每个 aggregate 只允许一个表驱动权威状态机。普通纵向切片生产代码预算 300–500 行，复杂 provider/状态机切片预计超过 800 行前先证明共享原语不足；拆文件、重命名、生成代码、删测试或降覆盖不计作减量。
  - [x] D72 选择方案 B 并加严结构约束：保留 D01–D71、现有 schema 语义和必要状态机；删除单用途 Protocol、内部对象图重复运行时校验、异常子类森林、中间 DTO、重复 HTTP/worker/persistence 门禁与大型重导出。不得引入 BaseService、万能 repository、元编程或生成代码；公共抽象原则上至少有三个真实调用方，Protocol 只用于外部边界或确有多个实现。
- [x] 14.3 以零新增生产层代码，把 D69 代表性非空默认租户迁移接入现有五阶段 `DefaultMigrationCommand`：实际消费 expand、resolved backfill、D68 `legacy_unattributed`、12 类 reconciliation、application/database-jobs enforcement 与 journal；在同一 MySQL 8 `inventory_management_test` 证明首跑到 contract 前权威边界、同 journal 空操作重跑、源漂移/孤儿/重复/Core ledger 非零拒绝和无 provider/打印副作用。最终 current-bundle 运行 `1 passed in 11.94s`；生产 Python 净新增 0，代表性测试由 875 行增至 1,034 行（+159），已显式记账且留待 14.4–14.6 参数化收敛。测试库随后恢复精确 12 表/8,536 行 source baseline。
- [ ] 14.4 第一轮收敛 shared kernel：统一 errors、transaction ownership/runner、canonical digest/evidence、idempotency 和 jobs 原语；按小范围调用者逐批迁移后删除旧实现，减少大型 `__init__.py` 重导出并改为定义模块直引。每批保持错误码、事务边界、digest、journal/outbox/attempt 语义和 SQL-backed 正反行为等价，不建立兼容层的永久双轨。
  - [x] transaction ownership 第一切片：新增唯一 `inventory_control.transactions` 定义 caller-owned explicit transaction 与 clean unit-of-work；默认迁移的 application enforce、tenant registration、warehouse、express type、planned logistics、structured address、logical accessory、empty history 和 D68 history 共 9 个服务改为直接依赖该定义，删除全部域内 `SessionTransactionOrigin` 判断和本地 transaction helper。共享文件已纳入 migration bundle，域错误类型、reader 后二次 clean 检查和 caller commit/rollback ownership 保持不变。最初为旧测试保留的 SQLite 锁物化已由下列 D71 切片删除。MySQL 8 聚焦矩阵 `30 passed`、bundle/command `8 passed`、当前代表性完整命令 `1 passed`；测试库已恢复 source baseline。
  - [x] canonical evidence 第一切片：新增唯一 `inventory_control.evidence` 字节级 JSON contract，并由 default phase evidence、D68 history、source baseline、schema qualification、reconciliation 和 express backfill 直接使用；各域继续独占 payload normalization、domain prefix、NaN 兼容策略和稳定错误类型，不合并不同摘要协议。共享文件纳入 migration bundle，纯摘要/qualification/bundle 矩阵 `55 passed`。
  - [x] transaction ownership 第二切片：backup artifact/lease persistence 与 acknowledgement persistence 改用同一 caller-owned clean transaction primitive，删除两套 origin/dirty/helper 实现；域错误码、只读方法和 caller commit/rollback ownership 不变，旧 SQLite lock 分支已由 D71 删除。
  - [x] transaction ownership 第三切片：schema-operation lease 与 fleet-migration persistence 改用同一 primitive，删除两套 origin/dirty/helper 实现；schema fence、锁顺序、CAS 和域错误不变。第二、三切片与 D68/phase adapter 的统一 MySQL 8 矩阵 `60 passed`；四个 persistence 文件相对续接 HEAD 合计由 2,712 行降至 2,601 行（净 `-111`），未把格式化计作收敛结果。
  - [x] transaction ownership 第四切片：backup/cloud freshness 与 queue operational signal adapters 改用共享 explicit transaction primitive，删除两套本地 origin 和 lock helper；无效 session 仍为 `OperationalInputError`，缺 caller transaction 仍为 `OperationalTransactionRequiredError`，双信号固定锁顺序和同事务回滚不变，旧 SQLite lock 分支已由 D71 删除。
  - [x] transaction ownership 第五切片：invitation、database-account mutation 与 SF account claim 三个 persistence 改用共享 explicit/clean primitive，删除三套 origin、dirty-state 和 lock helper；D47 锁顺序、lease/rotation CAS、SF claim fence、SAVEPOINT 和 caller commit/rollback ownership 不变，旧 SQLite lock 分支已由 D71 删除。第四、五切片 MySQL 8 聚焦矩阵 `61 passed`。
  - [x] transaction ownership 第六切片：共享 primitive 增加独立 `dirty_error` factory，使 root-key registry/lifecycle 与 recovery startup 在复用同一 clean-transaction 判定时仍保留“缺显式事务”和“脏事务”的不同稳定错误码；三个调用方及 primitive MySQL 8 聚焦矩阵 `34 passed`。本轮 8 个生产文件相对续接 HEAD 合计由 4,505 行降至 4,356 行（净 `-149`）；共享调用方由 13 增至 21，`SessionTransactionOrigin` 由 79 降至 63；当时剩余的 SQLite helper 已由下列 D71 切片全部删除。
  - [x] D71 SQL-backed backend 收口与 D72 第一组低风险删减：生产事务、job/outbox claim、数据库时钟、迁移 dialect guard 和配置只保留 MySQL 8/MariaDB 成功路径；删除 SQLite helper、claim seam 和默认 SQLite URI，数据库无关 Flask 测试改用不建立 socket 的 MySQL placeholder。`app`/`inventory_control`/migration/config/test Python 全扫描只剩一条“不支持 SQLite”的负向 URL；`SaasCoreHttpRuntimeBundle` 删除 149 行内部对象图重复校验，default phase adapters 删除 12 个单用途 Protocol，改为具体类型及一个三调用方 verifier 泛型。两个生产文件相对续接 HEAD 合计净 `-300` 行，组合/phase/guard 聚焦回归 `92 passed`。
  - [x] transaction ownership 第七/收口切片：jobs/outbox、identity、operations、provider integration、registration/lifecycle/routing 与 tenant shipping/relay/warehouse 共 30 个调用文件改用同一 primitive；共享入口以三个真实调用方所需的 `accept_nested` 保留 SAVEPOINT 语义。`app`/`inventory_control` 直接读取 `SessionTransactionOrigin` 的实现由 30 处降为唯一 shared-kernel 定义，51 个调用文件复用该定义；原错误类型/文案、clean 判定及 caller commit/rollback ownership 不变。jobs/expiry 的 6 套数据库时钟 reader 同时收敛到 `read_database_utc_datetime`。MySQL 8 最终共享控制面矩阵 `826 passed`，tenant shipping/relay/warehouse 聚焦矩阵 `191 passed`；另删除两个遗漏的模块私有 schema/锁 fixture。恢复码秒/微秒字段的现有 MySQL 舍入抖动以兼容既有表的最小 clamp 和确定性回归关闭，不改表或有效期。
  - [x] canonical digest/idempotency 第一切片：subscription renewal/adjustment、recovery release、schema-operation、default expand 与 MySQL grants 保留各自 payload/domain 语义，只把 compact sorted JSON + SHA-256 字节机制交给 `inventory_control.evidence`；旧协议的 ASCII/UTF-8 与 `allow_nan` 行为显式不变。outbox、provider-account、tenant integration 与 subscription-adjustment proof 删除 4 套本地 32-byte digest validator，10 个调用点复用同一 validator 并保留原域错误。带 domain prefix 的 binary digest、MAC、UUID/datetime normalization 未错误合并。MySQL 8 聚焦矩阵 `235 passed`，最终共享控制面矩阵 `826 passed`。
- [ ] 14.5 第二轮统一 HTTP runtime 与 worker/job framework：移除领域内重复的请求解析、AuthContext/权限、错误映射、transaction wrapper、lease/claim/retry/result 驱动；HTTP/worker 只做 adapter，业务与状态门禁留在 service/权威 state machine。用至少身份/租户路由、provider 副作用、后台 job 三类代表性纵向 contract 证明共享 runtime 不弱化隔离或 fail-closed。
- [ ] 14.6 第三轮统一 state machine、repository/CAS 与测试 fixture：每个 aggregate 用唯一表驱动 transition 定义，repository 只负责 CAS/持久化/冲突；把数据库、manifest、digest、迁移阶段和状态矩阵的重复 fixture 参数化，共享 contract 覆盖共同行为并保留领域专属边界。最终复测受影响矩阵、OpenSpec strict 和 diff check，给出基线对比；未达到生产 25%–35%、测试 20%–30% 的真实收缩目标时，列明剩余重复热点和下一批，不以删除有效测试或合并不相干领域补数。
  - [x] MySQL/MariaDB fixture 第一切片：把 control metadata 与 tenant/application metadata 合为一个 session-scoped `mysql_test_schema` owner，删除 32 组逐用例 `create_all/drop_all` 和最后 15 个模块私有 advisory-lock guard；每个用例只由中央 fixture 清空已知表。MySQL server-default 用例改为事务内显式 refresh，不依赖 SQLite `RETURNING` 或 detached ORM 残留。迁移模块聚焦 `185 passed`，全部共享 control-plane fixture 模块合并回归 `823 passed`，application/tenant 共享 fixture 矩阵 `321 passed`；本组中央及代表性测试文件相对续接 HEAD 净 `-162` 行。
