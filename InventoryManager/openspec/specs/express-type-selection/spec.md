# express-type-selection Specification

## Purpose
TBD - created by archiving change select-sf-express-type. Update Purpose after archive.
## Requirements
### Requirement: 数据持久化
...
- **Then** 字段类型为可空 `INTEGER`,并包含注释 "顺丰快递类型ID (1=特快,2=标快,263=半日达)"
...
#### Scenario: 应用默认值
- **Given** 新租赁记录未显式选择快递类型
- **When** 模型创建记录或顺丰下单服务读取空值
- **Then** 应用使用 `2` (标快)
- **And** 系统不依赖数据库 `server_default` 声明提供该默认值
...
- **And** `express_type_id` 的值在 `[1, 2, 263]` 范围内
- **And** 租赁记录存在
- **And** 该租赁尚未创建运单且未进入预约或发货状态
- **Then** 更新该租赁记录的 `express_type_id` 字段
- **And** 返回 `200 OK` 和响应体 `{ "success": true, "message": "快递类型已更新" }`

#### Scenario: 参数无效
- **Given** 前端发送 `PATCH /api/shipping-batch/express-type` 请求
- **When** `express_type_id` 不在 `[1, 2, 263]` 范围内(例如旧值 `6` 或 `99`)
...
#### Scenario: 运单创建后拒绝修改
- **Given** 租赁记录已有寄出运单号或状态为预约发货/已发货
- **When** 前端发送 `PATCH /api/shipping-batch/express-type` 请求
- **Then** 返回 `409 Conflict` 和错误消息 "运单已创建，快递类型不可修改"
- **And** 已保存的 `express_type_id` 保持不变
...
#### Scenario: 使用订单的快递类型
- **Given** 租赁订单的 `express_type_id` 已设置为 `263` (半日达)
- **When** 系统调用顺丰API为该订单下单
- **Then** 传递给顺丰API的 `expressTypeId` 参数值为 `263`
- **And** 同时传递 `specialDeliveryTypeCode` 参数值为 `263`
- **And** 日志记录所使用的半日达产品类型
...
#### Scenario: 非法存量值不得调用承运商
- **Given** 租赁订单存有 `6` 或其他不属于 `[1, 2, 263]` 的快递类型
- **When** 系统尝试创建顺丰运单
- **Then** 下单失败并提示重新选择快递类型
- **And** 系统不调用顺丰 provider
