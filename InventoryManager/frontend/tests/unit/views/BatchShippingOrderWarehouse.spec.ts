...
describe('BatchShippingOrderView warehouse isolation', () => {
  ...
  it('keeps the single shipping sheet bound to its rental warehouse contact', () => {
    ...
    expect(singleViewSource).not.toContain(['135102', '24947'].join(''))
    expect(singleViewSource).not.toContain(['松坪村竹苑', '9栋4单元415'].join(''))
    expect(singleViewSource).not.toMatch(/vacuumdust|张女士|小二微信/)
  })
})
