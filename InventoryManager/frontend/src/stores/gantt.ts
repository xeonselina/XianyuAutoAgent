...
export const useGanttStore = defineStore('gantt', () => {
  ...
  const rentalsByStatus = computed(() => {
    ...
    for (const rental of rentals.value) {
      const existing = grouped.get(rental.status)
      if (existing) existing.push(rental)
      else grouped.set(rental.status, [rental])
    }
    return grouped
  })

  const getRentalsByStatus = (status: string): readonly Rental[] => (
    rentalsByStatus.value.get(status) ?? []
  )
