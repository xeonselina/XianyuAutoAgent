...
class TestRentalAPIBundledAccessories:
    ...
    def test_create_rental_with_bundled_accessories_api(self, client, db_session):
        ...
        response = client.post(
            "/api/rentals", data=json.dumps(payload), content_type="application/json"
        )
    ...
    def test_create_rental_with_inventory_accessories_api(self, client, db_session):
        ...
        phone_holder = Device(
            name="手机支架-P-API01", serial_number="PHAPI-001", is_accessory=True
        )
        tripod = Device(name="三脚架-T-API01", serial_number="TAPI-001", is_accessory=True)
    ...
    def test_create_rental_with_mixed_accessories_api(self, client, db_session):
        ...
        phone_holder = Device(name="手机支架-P-API02", is_accessory=True)
    ...
    def test_update_rental_bundled_accessories_api(self, client, db_session):
        ...
        # 更新租赁，添加配套附件
        update_payload = {"includes_handle": True, "includes_lens_mount": True}
    ...
    def test_get_rental_includes_accessory_info(self, client, db_session):
        ...
        phone_holder = Device(
            name="手机支架-P-API03", serial_number="PHAPI-003", is_accessory=True
        )
...
class TestRentalAPIFinalUsagePeriodGuard:
    ...
    def test_logistics_facts_flow_through_create_update_and_status_routes(
        ...
        assert (
            created["planned_ship_out_date"]
            == (start_date - timedelta(days=1)).isoformat()
        )
        assert (
            created["planned_return_date"] == (end_date + timedelta(days=1)).isoformat()
        )
        ...
        assert (
            persisted["planned_ship_out_date"]
            == (new_start - timedelta(days=3)).isoformat()
        )
        assert (
            persisted["planned_return_date"]
            == (new_end + timedelta(days=3)).isoformat()
        )
