...
class TestRentalServiceBundledAccessories:
    ...
    def test_create_rental_with_bundled_accessories(self, app, db_session):
        ...
        with app.app_context():
            ...
            (
                main_rental,
                accessory_rentals,
            ) = RentalService.create_rental_with_accessories(rental_data)
    ...
    def test_create_rental_with_mixed_accessories(self, app, db_session):
        ...
        with app.app_context():
            # 创建设备
            main_device = Device(
                name="测试相机-A03", model="Test Camera", is_accessory=False
            )
            tripod = Device(name="三脚架-T01", model="Tripod", is_accessory=True)
    ...
    def test_get_all_accessories_for_display(self, app, db_session):
        ...
        with app.app_context():
            ...
            phone_holder = Device(
                name="手机支架-P02", serial_number="PH-002", is_accessory=True
            )
    ...
    def test_update_rental_with_accessories(self, app, db_session):
        ...
        with app.app_context():
            ...
            # 更新租赁，添加配套附件
            update_data = {"includes_handle": True, "includes_lens_mount": True}

            updated_rental = RentalService.update_rental_with_accessories(
                rental.id, update_data
            )
            ...
            with pytest.raises(RentalUsagePeriodConflictError) as captured:
                RentalService.update_rental_with_accessories(
                    second_id,
                    {
                        ...
                        "end_date": (first.end_date + timedelta(days=2)).isoformat(),
