...
| 租赁单（核心） | rental 租赁 订单 booking 预约 damage 损伤 附件 | `routes/rental_api.py` | `handlers/rental_handlers.py`; `services/rental/rental_service.py`, `services/rental_service.py`; `utils/rental_validator.py` | `models/rental.py`（含同单 RentalBooking / 幂等 RentalBookingRequest）, `models/rental_accessory.py` | PC `components/BookingDialog.vue`, `components/rental/*`（含共用 `BookingDeviceSelector.vue`） / 移动 `views/CreateRentalView.vue` `views/EditRentalView.vue` `components/BookingDeviceSelector.vue` |
