...
def _get_database_uri():
    ...
    if is_docker:
        ...
        # 在本地环境中，优先使用 DATABASE_URL_HOST；缺失时由应用工厂拒绝启动。
        return os.environ.get("DATABASE_URL_HOST") or os.environ.get("DATABASE_URL")
...
class TestingConfig(Config):
    ...
    TESTING = True
    # Database-free Flask tests may build routes without opening a socket.
    # SQL-backed tests must override this through the guarded MySQL test
    # configuration; an accidental query against the placeholder fails closed.
    SQLALCHEMY_DATABASE_URI = (
        "mysql+pymysql://unused@127.0.0.1:1/inventory_management_test"
    )
    SQLALCHEMY_ENGINE_OPTIONS = {}
