...
class GoofishBrowser:
        ...
        while self.is_running:
            try:
                ...
                for message in current_messages:
                    message_key = f"{message.get('text', '')}_{message.get('timestamp', '')}"
                    
                    if message_key not in previous_messages:
                        previous_messages.add(message_key)
                        
                        if self.message_callback:
                            await asyncio.create_task(self._handle_new_message(message))
    ...
    async def _handle_new_message(self, message: Dict):
        """处理新消息"""
        try:
            if self.message_callback:
